...
def generate_repo_map(
        ...
        if files and files[0]:
            ...
            def build_tree_from_files(files: list, max_files: int) -> list:
                ...
                other_dirs = [
                    ...
                    if d not in PRIORITY_DIRS and d != ""
                ]
            ...
            shown_files = len(
                [line for line in tree if "├──" in line and not line.endswith("/")]
            )
    ...
    def walk_dir(
        current_dir: str, prefix: str = "", depth: int = 0, max_depth: int = 5
    ):
        ...
        for i, d in enumerate(sorted_dirs):
            ...
            walk_dir(
                os.path.join(current_dir, d), prefix + extension, depth + 1, max_depth
            )
