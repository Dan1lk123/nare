...
SYSTEM_PROMPT_HEAD = """
[NARE v0.3.0 - Tool Format: JSON ONLY - No XML tags allowed]
...
CRITICAL:
- Tool calls MUST use JSON format inside <tool_call> tags
- DO NOT use raw XML tags like <read_file>, <bash_command>, <write_file>
- ALWAYS use: <tool_call>{"name": "read_file", "args": {"path": "file.py"}}</tool_call>
...
CRITICAL - AVOID VAGUE TASKS:


- If task is vague ("find all bugs", "check everything"), ask user for specific target.


- NEVER fabricate problems to justify reading files.


- If you don't know what to look for, ask instead of guessing.


- Vague exploration wastes tokens. Be specific or ask for clarification.
...
- Use grep/find to locate code before reading entire files


- After 3 read-only operations, you MUST either edit code or provide final answer


- Don't re-read files you've already seen - check previous OBSERVATION blocks
...
- NEVER assume file contents without reading it first


- NEVER assume function signatures without checking the code


- If you're not 100% certain, use read_file or grep to verify


- When user says "fix function X", ALWAYS read the file containing X first


- Test your changes with bash commands to verify they work


- If a file path doesn't exist in your observations, you DON'T know what's in it
...
def render_system_prompt(registry: ToolRegistry, working_dir: str, repo_map: Optional[str] = None) -> str:
    ...
    # Load NARE.md if exists (project-specific rules to save tokens)
    import os
    nare_md = ""
    nare_path = os.path.join(working_dir, 'NARE.md')
    if os.path.exists(nare_path):
        try:
            with open(nare_path, 'r', encoding='utf-8') as f:
                content = f.read().strip()
                if content:
                    nare_md = f"\n\n# PROJECT RULES (from NARE.md)\n{content}\n"
        except Exception:
            pass
...
class AgentLoop:
    ...
    async def run(
        ...
        *,
        ...
        # Track read-only operations to prevent endless exploration
        consecutive_reads = 0
        max_consecutive_reads = 3  # Max 3 reads without write/edit action
        ...
        while True:
            ...
            if parsed.kind == "malformed":
                ...
                # Don't emit parse errors to user - they're internal
            ...
            if thinking_text and len(thinking_text) > 10:
                ...
                # LLM provided reasoning - emit it (without prefix, renderer adds it)


                self.bus.emit(Thought(text=thinking_text[:200]))
            ...
            if not parsed.tool_args:
                ...
                # Invalid tool call - internal validation
            ...
            if parsed.tool_name in required_params:
                ...
                if missing:
                    ...
                    # Missing required args - internal validation
            ...
            if parsed.tool_name == 'read_file':
                ...
                # Track consecutive read operations
                consecutive_reads += 1
                ...
                if consecutive_reads > max_consecutive_reads:
                    ...
                    result = ToolResult(
                        ok=False,
                        error=(
                            f"STOP. You've performed {consecutive_reads} consecutive read operations without taking action. "
                            f"If you're exploring to find bugs, you must specify WHAT bug you're looking for. "
                            f"Vague exploration wastes tokens. Either:\n"
                            f"1. Specify a concrete problem to investigate\n"
                            f"2. Make an edit based on what you've already read\n"
                            f"3. Provide a final answer if the task is complete"
                        )
                    )
                else:
                    # Track read count
                    ...
                    if read_counts[file_path] > 2:
                        ...
                        # File read limit exceeded - internal block
                        ...
                        # Cache hit - no need to emit to user
                    else:
                ...
                # Reset consecutive reads for non-read operations
                if parsed.tool_name in ('grep', 'find_function', 'find_class', 'search_for_hunk', 'list_dir', 'bash', 'find_files'):
                    consecutive_reads += 1
                else:
                    consecutive_reads = 0

                # Hard stop after max consecutive reads
                if consecutive_reads > max_consecutive_reads:
                    from nare.tools.builtin.base import ToolResult
                    result = ToolResult(
                        ok=False,
                        error=(
                            f"STOP. You've performed {consecutive_reads} consecutive search/read operations without taking action. "
                            f"This task is too vague. You must either:\n"
                            f"1. Make a concrete edit to fix a specific problem\n"
                            f"2. Provide a final answer summarizing what you found\n"
                            f"3. Ask the user for a more specific task\n\n"
                            f"Endless exploration wastes tokens."
                        )
                    )
            ...
            if result.ok and parsed.tool_name in ('edit_file', 'write_file'):
                ...
                if file_path and file_path in read_cache:
                    ...
                    # Cache invalidated - internal operation

                # Reset consecutive reads counter on write action
                consecutive_reads = 0
                ...
                try:
                    ...
                    if os.path.exists(os.path.join(self.working_dir, '.git')):
                        ...
                        # Auto-committed - internal operation
                except Exception as e:
            ...
            if not result.ok:
                ...
                if parsed.tool_name in ('find_function', 'find_class', 'search_for_hunk', 'grep'):
                    ...
                    if len(recent_failed_searches) >= 3:
                        ...
                        # Loop detected - internal protection
                ...
                if parsed.tool_name == 'edit_file':
                    ...
                    if result.summary and ('+0' in result.summary and '-0' in result.summary):
                        ...
                        # Edit made no changes - internal warning
                    ...
                    if len(recent_updates) >= 1:
                        ...
                        # Repeatedly updating todos - internal warning
                ...
                if is_no_op:
                    ...
                    # No-op tool call - internal warning
            ...
            # Auto-compact: keep only last 10 observations to save tokens
            # Keep USER REQUEST and PLAN at the beginning
            if len(observations) > 12:
                # Find indices of USER REQUEST and PLAN
                user_req_idx = next((i for i, o in enumerate(observations) if o.startswith("# USER REQUEST")), None)
                plan_idx = next((i for i, o in enumerate(observations) if o.startswith("# PLAN")), None)

                # Keep first 2 (USER REQUEST + PLAN) and last 10 observations
                preserved = []
                if user_req_idx is not None:
                    preserved.append(observations[user_req_idx])
                if plan_idx is not None:
                    preserved.append(observations[plan_idx])

                preserved.extend(observations[-10:])
            ...
            if self.budget.iterations >= 5:
                ...
                if len(recent_actions) >= 4:
                    ...
                    if most_common and most_common[0][1] >= 3:
                        ...
                        # Action loop detected - internal protection
...
def _make_final_answer_streamer(
    ...
    state = {
        ...
        "in_tool_call": False,  # Track if we're inside <tool_call>
        ...
        "tool_open_pattern": r'<tool_call\s*>',  # Regex pattern for <tool_call>


        "tool_close_pattern": r'</tool_call\s*>',  # Regex pattern for </tool_call>
    ...
    def _push(delta: str) -> None:
        ...
        # ALWAYS filter out <tool_call> blocks
        while True:
            if not state["in_tool_call"]:
                match = re.search(state["tool_open_pattern"], state["buf"])
                if match:
                    state["in_tool_call"] = True
                    # Emit content before <tool_call> if in final_answer mode
                    before_tool = state["buf"][:match.start()]
                    after_tool_tag = state["buf"][match.end():]

                    if state["in_final"] and before_tool:
                        if len(before_tool) > TAIL:
                            emit = before_tool[:-TAIL]
                            on_text(emit)
                            state["buf"] = before_tool[-TAIL:] + after_tool_tag
                        else:
                            on_text(before_tool)
                            state["buf"] = after_tool_tag
                    else:
                        state["buf"] = after_tool_tag
                    continue
                else:
                ...
                match = re.search(state["tool_close_pattern"], state["buf"])
                if match:
                    # Skip everything up to and including </tool_call>
